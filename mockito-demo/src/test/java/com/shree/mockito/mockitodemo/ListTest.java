...
class ListTest {
	...
	void testGet_GenericParameter() {
		List listMock = mock(List.class);
		when(listMock.get(Mockito.anyInt())).thenReturn("Some String");  // If one parameter is generic then all parameter 
		assertEquals("Some String",listMock.get(0));                     // has to be generic.
